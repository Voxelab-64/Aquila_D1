...
#include "lcd/marlinui.h"
  ...
  #endif
#endif
  #include "lcd/e3v2/common/i2c_eeprom.h"
#if HAS_ETHERNET
...
void startOrResumeJob() {
  ...
  inline void finishSDPrinting() {
    if (queue.enqueue_one(F("M1001"))) {  // Keep trying until it gets queued
      ...
      // TERN_(DGUS_LCD_UI_ORIGIN, ScreenHandler.SDPrintingFinished());

      // TERN_(DGUS_LCD_UI_ORIGIN, DGUSScreenHandler::DWIN_CompletedMaticHate());
    }
  }
...
void idle(bool no_stepper_sleep/*=false*/) {
  ...
  TERN_(DGUS_LCD_UI_ORIGIN, ScreenHandler.HandleManualhotend());
  // Run i2c Position Encoders
  #if ENABLED(I2C_POSITION_ENCODERS)
  {
...
void setup() {
  ...
  #else
    delay(800);   // Delay here or init later in the boot process
    SETUP_RUN(ui.init());
  ...
  #endif
    TERN_(DGUS_LCD_UI_ORIGIN, DGUSScreenHandler::filament_flag_select());
    TERN_(DGUS_LCD_UI_ORIGIN, DGUSScreenHandler::lcd_select_language());
    TERN_(DGUS_LCD_UI_ORIGIN, DGUSScreenHandler::HandleBeepLockUnlockSelect());                                  
  #if HAS_DWIN_E3V2_BASIC
    SETUP_LOG("E3V2 Init");
