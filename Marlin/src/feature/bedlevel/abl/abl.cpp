...
float bilinear_z_offset(const xy_pos_t &raw) {
  ...
  static float z1, d2, z3, d4, L, D,R,gx,gy;
  ...
  if (prev.x != rel.x) {
    ...
    gx = constrain(FLOOR(ratio.x), 0, ABL_BG_POINTS_X - (FAR_EDGE_OR_BOX));
  ...
  if (prev.y != rel.y || lastg.x != thisg.x) {
    ...
    if (prev.y != rel.y) {
      ...
      ratio.y = rel.y * ABL_BG_FACTOR(y);
        gy = constrain(FLOOR(ratio.y), 0, ABL_BG_POINTS_Y - (FAR_EDGE_OR_BOX));
      ratio.y -= gy;
  ...
  //   SERIAL_ECHOLNPGM("bilinear_start x=", bilinear_start.x, " bilinear_start y=", bilinear_start.y);
  //   SERIAL_ECHOLNPGM("bilinear_grid_factor x=", bilinear_grid_factor.x, " bilinear_grid_factor y=", bilinear_grid_factor.y);
  ...
  //   SERIAL_ECHOLNPGM(" gx=", gx, " gy=", gy);
  //   SERIAL_ECHOLNPGM(" z1=", z1, " d2=", d2, " z3=", z3, " d4=", d4);
